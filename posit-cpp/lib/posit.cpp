...
Posit Posit::add(const Posit& p) const
{
    ...
    unpacked_t aup = unpack_posit(mBits, mNbits, mEs);      // a unpack posit
    unpacked_t bup = unpack_posit(p.mBits, p.mNbits, p.mEs);    // b unpack posit
    unpacked_t up = op2_add(aup, bup);   // add a and b
...
{
    std::cout << "this is getFloat function" << std::endl;
    if (isZero()) {
        return 0.f;
    } else if (isNar()) {
        return 0.f / 0.f;
...
{
    // std::cout << "this is setBits function" << std::endl;
    // std::cout << "bits: " << bits << std::endl;
    // std::cout << "mNbits: " << mNbits << std::endl;
    mBits = LSHIFT(bits, POSIT_WIDTH - mNbits); // shift to the left to align the bits 
    // std::cout << "mBits: " << mBits << std::endl;
}

POSIT_UTYPE Posit::getBits()
{
    // std::cout << "this is getBits function" << std::endl;
    return RSHIFT(mBits, POSIT_WIDTH - mNbits);
}

void Posit::print()
{
    // std::cout << "\nthis is print function" << std::endl;
